...
A：
- Raft：当需要强一致、强持久；能接受更高写延迟与多数副本可用的前提
- fetch：当更看重吞吐/时延、可接受最终一致；副本异步追leader
...
Q：如果zk和broker产生分区zk标志，broker下线，生产者还是通过缓存连接上broker怎么办？
   如果broker是正常运行的？如果zk和生产者也无法感知？会有消息丢失吗？为什么？如果运维的同学后台将这个broker kill掉了呢？ 

A：
1. 问题：ZK认为Broker下线，选出新Leader,但Producer无法感知，仍连上旧Leader，新Leader可能没有Producer刚发送的消息。旧Leader会正常写入并存到本地，但新的消费者请求会发送到新Leader,新Leader并没有这部分消息。

解决：Broker端主动检测ZK连接，下线尝试重连

2. 问题：运维同学后台kill -9 broker进程,zk和Producer都无法感知，网络层面可能成功(TCP连接建立)，但Broker进程已经不存在，无法处理请求，可能发生连接超时、连接重置、无响应，从而丢失消息。

解决：缺乏主动健康检测，只有在Push失效时才会更新缓存，因此可以添加客户端主动检测Broker状态；
或者实现重试机制。

Q：推和拉模式各有什么优缺点？单点故障怎么解决？

A：
- 推（S->C）：获取消息及时，但是无法确定consumer端的接收速度，可能会把consumer端的缓冲区打满，导致消息丢失
- 拉（c->S）：不需要估计consumer的接收速度，但是消息获取不及时，当消息比较少的情况下，需要consumer不断轮询服务器是否有消息

Q：怎么实现的？参照kafka,和kafka的相似点与改进？

A：
参照kafka：
- 整体架构：Broker作为消息存储和转发的核心节点，每个Broker管多个topic和partition；Topic-Partition模型：topic为消息的主题分类，partition是topic的分片，支持并行消费
- 消费者组概念：consumer group管理多个消费者，同一个partition只能被组内一个消费者消费
- 消费模式：PTP - 点对点 类似kafka的exactly-once语义，PSB - 发布订阅 类似kafka的广播消费
- offset概念：维护消费位点offset,实时更新
- ZK存储位点：仿照kafka早期版本使用ZK存储元数据，kafka后来后来自己使用raft协议内置了一个类似zookeeper
- fetch机制
- 消息写入磁盘文件，顺序写

不同：
- 副本同步机制：ISR机制改为raft，采用multi-raft的优化，三个机器就可以组三个raft集群，每个机器负责不同的分片的服务即leader，每个机器都是leader同时又是另外两个机器的follower。

优点：并行化、负载分担，适合大规模分片存储系统

缺点：Leader调度复杂，跨分片一致性难

- 内存缓冲+批量写入，kafka：零拷贝+页缓存
...
A：
- 负载均衡：数据分布相对均匀，减少热点
- 扩展性：支持动态添加/删除节点，数据迁移最小化
- 容错性：节点故障时自动故障转移，系统可用性高
- 稳定性：节点变化时，大部分数据无需重新分配

Q：还有哪里需要优化？

A：
- 读写优化，使读消息不走leader可通过follower读取，这个优化etcd的raft有做可以参考
- 分片动态迁移（根据CPU，磁盘的负载情况动态迁移，可参考raft论文中的集群变换）
- 消息的类型，可扩展不同的消息类型，例如：灰度消息、死信队列、延时消息、事务消息等

Q：消费者进行消费时有无容错机制？

A：
- 重试机制：发送失败重试，在SendOneBlock中实现重试逻辑，超时三次将认为消费者掉线
- 状态管理：消息状态跟踪和恢复
- 心跳机制：与消费者建立连接时就开一个发送心跳的协程，消费者掉线后就会从消费者组中移除
...
A：
- 分区状态管理
- 副本分配和负载均衡，利用一致性哈希算法选择broker副本，确保副本分配均匀，支持动态扩缩容
- Leader选举协调，更新zk中的LeaderBroker
- Broker健康检查
...
内联索引文件结构：可以跳过不相关的数据，只读取元数据进行判断，支持索引查询，知道消息边界

Q：分布式消息队列在数据量大时如何处理？

A：
- 水平分区，每个分区可以并行处理消息，消息分散到多个Broker
- 消息顺序写+批量写入磁盘
- 消费端并行，多个消费者同时消费一个topic的消息
- Producer端消息压缩，减少网络传输，提高传输速度，减轻Broker压力

Q：SSTable，读优化还是写优化？为什么是写优化？SSTable写逻辑？怎样确保数据有序的？

A：

写优化：
- 批量写入收集大量数据后一次性写入，减少磁盘寻道
- 顺序写入：数据按顺序写入，避免随机写入
- 不可变文件：写入完成后文件不可修改，避免复杂的并发控制
- 压缩优化，减少实际写入的数据量
...
A：
- 功能完整性测试：测试生产者共鞥，消费者功能，Partition管理，文件I/O
- 一致性测试：Raft一致性，一致性哈希，配置一致性
- 集成测试：端到端流程、集群协调、状态管理
- 边界条件测试：资源分配、文件操作、错误管理

改进：
- 压力测试：添加高并发、大数据量测试
- 故障测试：测试网络分区、节点崩溃等异常情况

Q：写项目有什么困难？

A：
- 分布式系统复杂性：一致性问题、状态管理复杂性（SetPartitionState）
- 性能优化：高并发挑战、I/O性能优化
- 容错和可靠性：数据丢失防护、网络分区处理
- 架构设计复杂性：控制面与数据面分离、一致性哈希
- 测试和调试困难：分布式系统测试、性能测试
- 技术选型困难：一致性协议选择、存储方案选择